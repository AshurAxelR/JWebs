...
http://parsedown.org
*/
package com.xrbpowered.jwebs.markdown;
...
public class JParsedown {
	...
	protected class BlockList extends Block {
		...
		public Block continueBlock(Line line) {
			...
			Matcher m;
			if(line.indent < requiredIndent && ((ordered && (m = Pattern.compile("^[0-9]++" + markerTypeRegex + "(?:[ ]++(.*)|$)").matcher(line.text)).find()) ||
		...
		public Inline inline(String text, String context) {
			...
			if((m = Pattern.compile("^[(]\\s*+((?:[^ ()]++|[(][^ )]+[)])++)(?:[ ]+(\"[^\"]*+\"|\\'[^\\']*+\'))?\\s*+[)]").matcher(remainder)).find()) {
				...
				if((m = Pattern.compile("^\\s*\\[(.*?)\\]").matcher(remainder)).find()) {
					definition = !m.group(1).isEmpty() ? m.group(1) : ((LineElementsHandler) element.handler).text;
